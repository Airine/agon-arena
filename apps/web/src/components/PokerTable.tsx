...
export default function PokerTable({
  ...
  useEffect(() => {
    ...
    if (!gameState) {
      // Empty state label
      ctx.fillStyle = isTerminalEmptyState ? '#555570' : '#22DD88';
      ctx.font = `${isTerminalEmptyState ? '400' : '500'} ${W < 480 ? 13 : 15}px 'JetBrains Mono', monospace`;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(emptyLabel ?? 'Waiting for game...', cx, cy);
      return;
    }
    ...
    if (lastAction) {
      ...
      if (actor) {
        ...
        const color = ACTION_COLORS[actionType] ?? '#AAAACC';
  ...
  }, [gameState, width, height, emptyLabel, isTerminalEmptyState]);
